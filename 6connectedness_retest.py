...
OUTPUT_CSV = "apr8analysispipeline/retest_trial/cns_output.csv"
...
def compute_cns_composite(df, label):
    df[f"CNS_Composite_{label}"] = df[cns_cols].mean(axis=1)
    df[f"CNS_Composite_{label}_Norm"] = df[f"CNS_Composite_{label}"] / 7
    return df[["Email", f"CNS_Composite_{label}_Norm"]]
